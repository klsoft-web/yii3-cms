            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                <ul class="navbar-nav me-auto">
                    ...
                    foreach ($siteManager->getTopNavItems() as $navItem) {
                        $url = $navItem['nav_item_type'] === NavItemType::Slug ? '/' . ((string)$navItem['value']) : ((string)$navItem['value']);
                        echo Html::li(Html::a((string)$navItem['name'], $url, ['class' => 'nav-link' . ($url === $currentRoute->getUri()?->getPath() ? ' active' : '')]))->attributes(['class' => 'nav-item']);
                    }
                    ?>
                </ul>
            </div>
    ...
    <div class="container">
        <?php if (count($siteManager->getBottomNavs()) > 0): ?>
            <div class="row">
                <?php
                /** @var string $navName */
                foreach ($siteManager->getBottomNavs() as $navName => $navItems): ?>
                            ...
                            foreach ($navItems as $navItem): ?>
                                <li>
                                    <a class="link-secondary text-decoration-none"
                                       href="<?= $navItem['nav_item_type'] === NavItemType::Slug ? '/' . ((string)$navItem['value']) : ((string)$navItem['value']) ?>"><?= ((string)$navItem['name']) ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
