...
final class UserForm extends FormModel
{
    ...
    {
        $result = new Result();
        $user = $this->userAdminRepository->findByName($this->name);
        if ($user !== null &&
            $user->getId() !== $this->id) {
    ...
    {
        $result = new Result();
        $user = $this->userAdminRepository->findByEmail($this->email);
        if ($user !== null &&
            $user->getId() !== $this->id) {
            $result->addError($this->translator->translate(
                App::THE_RECORD_WITH_THE_FIELD_NAME_ALREADY_EXISTS,
                ['field_name' => strtolower($this->translator->translate(App::EMAIL))]));
        }
        return $result;
    }
