...
<?= $gridView
    ...
    ->bodyRowAttributes(static function(array|object $data) use ($canUserUpdateCategory, $urlGenerator) {
        /** @var Category $category */
        $category = $data;
        return $canUserUpdateCategory ? ['class' => 'entity-item-editable', 'data-edit-url' => $urlGenerator->generate('admin/update-category', ['id' => $category->getSlug()?->getId()])] : [];
    })
    ->columns(
        new CheckboxColumn(
            ...
            content: static function(Checkbox $checkbox, DataContext $context) {
                ...
                return $checkbox->value($category->getSlug()?->getId());
            },
