...
<?= $gridView
    ...
    ->bodyRowAttributes(static fn(array|object $data) => $canUserUpdateRole ? ['class' => 'entity-item-editable', 'data-edit-url' => $urlGenerator->generate('admin/update-role', ['id' => (string)((array)$data)['name']])] : [])
    ->columns(
        new CheckboxColumn(
            headerAttributes: ['id' => 'entities-checkbox-header'],
            inputAttributes: ['class' => 'entity-checkbox-selector'],
            content: static fn(Checkbox $checkbox, DataContext $context) => $checkbox->value((string)((array)$context->data)['name']),
