...
<div class="mb-3">
    <label for="category_id" class="form-label"><?= $translator->translate(App::CATEGORY) ?></label>
    <?= Html::select('category_id')->addAttributes([
        'id' => 'status',
        'class' => 'form-select',
        'placeholder' => ''
    ])->value($form->category_id)
        ->options(...array_map(fn(?Category $category) => $category !== null ? Option::tag()->value($category->getSlug()->getId())->content($category->getName()) : Option::tag()->value(null), $form->categories)) ?>
</div>
<div class="mb-3">
    <div class="card">
