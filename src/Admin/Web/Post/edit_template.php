...
<div class="mb-3">
    ...
    <?php foreach ($form->categories as $catId => $data): ?>
        <div class="form-check">
            ...
            <label class="form-check-label"><?= (string)$data['name'] ?></label>
        </div>
    <?php endforeach; ?>
</div>
<div class="mb-3">
...
<div class="mb-3">
    ...
    <?= Html::select('status')->addAttributes([
        ...
        ->options(
             (new Option)->value(PostStatus::Active->value)->content(PostStatus::Active->name),
             (new Option)->value(PostStatus::Inactive->value)->content(PostStatus::Inactive->name)) ?>
</div>
