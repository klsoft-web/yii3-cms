...
<?= $gridView
    ...
    ->bodyRowAttributes(static function(array|object $data) use ($canUserUpdatePost, $urlGenerator) {
        /** @var Post $post */
        $post = $data;
        return $canUserUpdatePost ? ['class' => 'entity-item-editable', 'data-edit-url' => $urlGenerator->generate('admin/update-post', ['id' => $post->getSlug()?->getId()])] : [];
    })
    ->columns(
        new CheckboxColumn(
        ...
        new DataColumn(
            ...
            content: static fn(Post $post) => $post->getDateTime()->format((new IntlDateFormatter(
                $applicationParams->locale,
                IntlDateFormatter::SHORT,
                IntlDateFormatter::SHORT))->getPattern()),
        ),
        new DataColumn(
            property: 'name',
