...
final class PostController
{
        ...
        try {
            ...
            if ($this->formHydrator->populateFromPost($form, $request)) {
                if ($this->formHydrator->validate($form)->isValid() &&
                    ...
                    $entityChangedResult = $post->getSlug()->getId() !== $form->slug ? $this->postRepository->updateSlug($post, $this->createSlug($form), $form->categories) : $this->postRepository->save($post, $form->description === '', $form->categories);
                ...
                if ($this->validate($id, $post, $form) &&
                    ...
                    $this->setCategories(array_map(fn($postCategory) => $postCategory->getCategory()->getSlug()->getId(), $this->postRepository->getPostCategories($post)), $form);
                } else {
    ...
    private function setCategories(array $postCategories, PostForm $form): void
    {
        ...
        foreach ($categories as $category) {
            $postBelongToCategories[$category->getSlug()->getId()] = ['name' => $category->getName(), 'isPostBelongsToCategory' => in_array($category->getSlug()->getId(), $postCategories)];
        }
