...
final class PostController
{
            ...
            ->render(
                ...
                [
                    'dataReader' => (new DoctrineDataReader(
                        ...
                        ->withSort(Sort::any(['date_time'])->withOrder(['date_time' => 'desc'])),
                    'canUserCreatePost' => $this->user->can(RbacPermission::CREATE_POST),
                    'canUserUpdatePost' => $this->roleRepository->userHasPermission($userId, RbacPermission::UPDATE_POST),
        ...
        try {
            ...
            if ($this->formHydrator->populateFromPost($form, $request)) {
                if ($this->formHydrator->validate($form)->isValid() &&
                    ...
                    $entityChangedResult = $post->getSlug()?->getId() !== $form->slug ? $this->postRepository->updateSlug(
                        $post,
                        $this->createSlug($form),
                        $form->description === '',
                        $form->categories) :
                        $this->postRepository->save($post, $form->description === '', $form->categories);
                    $this->eventDispatcher->dispatch(new EntityChanged($entityChangedResult->entity, $entityChangedResult->eventType, (int)$userId));

                    return $this->responseFactory->createResponse()
                        ->withStatus(Status::FOUND)
                        ->withHeader('Location', $this->urlGenerator->generate('admin/posts'));
                }
                $this->setCategories($form->categories, $form);
            } else {
                if ($this->validate($id, $post, $form)) {
                    $postSlug = $post->getSlug();
                    if ($postSlug !== null) {
                        $form->id = $postSlug->getId();
                        $form->slug = $postSlug->getId();
                        ...
                        $this->setCategories(array_map(fn(PostCategory $postCategory) => $postCategory->getCategory()->getSlug()?->getId(), $this->postRepository->getPostCategories($post)), $form);
                    } else {
                        $this->setCategories([], $form);
                    }
                } else {
                    $this->setCategories([], $form);
                }
    ...
    private function setCategories(array $postCategories, PostForm $form): void
    {
        $postBelongToCategories = [];
        $categories = $this->categoryRepository->getAll();
        foreach ($categories as $category) {
            /** @var string $categorySlugId */
            $categorySlugId = $category->getSlug()?->getId();
            $postBelongToCategories[$categorySlugId] = ['name' => $category->getName(), 'isPostBelongsToCategory' => in_array($categorySlugId, $postCategories)];
        }
        $form->categories = $postBelongToCategories;
    }
