...
final class NavController
{
        ...
        return $this->viewRenderer
            ->render(
                ...
                [
                    ...
                    'canUserUpdateNav' => $this->roleRepository->userHasPermission($userId, RbacPermission::UPDATE_NAVIGATION),
        ...
        #[RouteArgument('id')] ?int $id = null): ResponseInterface
    {
        ...
        try {
            $nav = new Nav();
            if ($id !== null) {
                $nav = $this->navRepository->find($id);
                if ($nav === null) {
                    throw new Exception($this->translator->translate(App::THE_RECORD_WAS_NOT_FOUND));
                }
            }
    ...
    public function findEntities(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($this->formHydrator->populateFromPostAndValidate($form, $request)) {
            if ($form->entity_type === 'Category') {
                $entities = array_map(fn(Category $category) => ['id' => $category->getSlug()?->getId(), 'name' => $category->getName()], $this->categoryRepository->findAllByNameLikeTo($form->search, 0, 5));
            } else {
                $entities = array_map(fn(Post $post) => ['id' => $post->getSlug()?->getId(), 'name' => $post->getName()], $this->postRepository->findAllPagesByNameLikeTo($form->search, 0, 5));
            }
        }
