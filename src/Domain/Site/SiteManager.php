...
use App\Data\Site\PostCategoryDataReader;
...
final readonly class SiteManager implements SiteManagerInterface
{
    ...
    public function findEntityBySlug(?string $slug): ?object
    {
        if ($slug === null) {
            try {
                return (object)$this->cache->getOrSet(
                    self::HOME_PAGE,
                    function () {
                        return $this->getHomePagePost();
                    });
            } catch (Throwable) {
    ...
    private function getHomePagePost(): Post
    {
        $post = null;
    ...
    {
        return (array)$this->cache->getOrSet(
            self::TOP_NAV,
            function () {
                $navs = (array)$this->entityManager->createQueryBuilder()->select('n')
                    ->from(Nav::class, 'n')
    ...
    {
        return new PostCategoryDataReader($this->entityManager, $category);
    }
    ...
    public function entityChanged(object $entity, EntityEventType $eventType): void
    {
        if ($entity instanceof Post) {
            if ($entity->getSlug()?->getId() === $this->getHomePageSlug()) {
                ...
                /** @var string $slugId */
                $slugId = $entity->getSlug()?->getId();
                $this->findNavItemBySlugThenRemoveNavCache($slugId);
            }
